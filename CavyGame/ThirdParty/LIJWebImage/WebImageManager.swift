...
class WebImageManager: NSObject {
    ...
    var waitimageCache = NSMutableDictionary()
    ...
    func isDownloading(path: String)->Bool{
    ...
    func getImg(path: String)->UIImage{
        return self.imageCache[path] as! UIImage
    }
    
    func downloadWebImage(path: String, network needfromNetwork : Bool, complition: (image: UIImage) -> ()){
        ...
        if let _ = self.operationCache[path] as? NSOperation {
            ...
            var waitImage = Array<(image: UIImage) -> ()> ()
            
            if let imeArray = self.waitimageCache[path] as? Array<(image: UIImage) -> ()> {
                
                waitImage = imeArray
                
            }
            
            waitImage.append(complition)
            
            waitimageCache.setValue((waitImage as? AnyObject), forKey: path)
