...
CGFloat const SDRefreshViewDefaultHeight = 35.0;//70.0f;
CGFloat const SDActivityIndicatorViewMargin = 100.0f;
...
- (void)layoutSubviews
{
    ...
    _activityIndicatorView.center = CGPointMake(self.sd_width * 0.5 - SDActivityIndicatorViewMargin, self.sd_height * 0.5 + 3);
...
- (void)setRefreshState:(SDRefreshViewState)refreshState
{
    ...
    switch (refreshState) {
        ...
        case SDRefreshViewStateRefreshing:
        {
            ...
            if ([self isKindOfClass:[SDRefreshFooterView class]]) {
                _textIndicator.text = localizedString(@"SDRefreshViewLoadMoreingStateText");
                
            }else{
                _textIndicator.text = localizedString(@"SDRefreshViewRefreshingStateText");
            }
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        ...
        {
            if ([self isKindOfClass:[SDRefreshFooterView class]]) {
                _textIndicator.text = localizedString(@"SDRefreshViewWillLoadMoreStateText");
                
            }else{
                _textIndicator.text = localizedString(@"SDRefreshViewWillRefreshStateText");
            }
        ...
        case SDRefreshViewStateNormal:
        {
            ...
            if ([self isKindOfClass:[SDRefreshFooterView class]]) {
                _textIndicator.text = localizedString(@"SDRefreshViewWillLoadMoreNomalStateText");
                
            }else{
                _textIndicator.text = localizedString(@"SDRefreshViewWillRefreshNomalStateText");
            }
            
            _timeIndicator.text = [NSString stringWithFormat:@"%@%@", localizedString(@"SDRefreshViewLastTime"), [self lastRefreshingTimeString]];
